...
float* multVector(float* a, float* b) {
...
//======================================================================
{
	...
	float T[16] = { 1.0f, 0, 0, -viewer_posn[1],
					  0, 1.0f, 0, -viewer_posn[2],
					  0, 0, 1.0f, -viewer_posn[0],
					  0, 0, 0, 1 };

	float R[16] = { -cos(Maze::To_Radians(viewer_dir)), 0, sin(Maze::To_Radians(viewer_dir)), 0,
					  0, 1.0f, 0, 0,
	...
	float P[16] = { 0.01f / r, 0, 0, 0,
				   0, 0.01f / r, 0, 0,
				   0, 0, (-(200.0f + 0.01f)) / (200.0f - 0.01f), (-2.0f * 200.0f * 0.01f) / (200.0f - 0.01f),
				   0, 0, -1.0f, 0 };

	float* worldToViewMatrix = multMatrix(R, T);
	float* worldToScreenMatrix = multMatrix(P, worldToViewMatrix);
	...
	for (int i = 0; i < (int)this->num_edges; i++) {
		...
		if (this->edges[i]->opaque) {
			...
			cell.push_back(multVector(worldToScreenMatrix, point0));
			cell.push_back(multVector(worldToScreenMatrix, point1));
			cell.push_back(multVector(worldToScreenMatrix, point2));
			cell.push_back(multVector(worldToScreenMatrix, point3));
	...
	free(worldToViewMatrix);
	free(worldToScreenMatrix);
}
